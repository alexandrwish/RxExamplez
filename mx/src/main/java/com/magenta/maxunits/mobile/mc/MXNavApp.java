...
public class MXNavApp implements Routeable.Listener {
    ...
    static final String FORMATTED_STRING = "Distance between A [%f ; %f] and B [%f ; %f] = %f.";
    static final String FORMATTED_EXCEPTION = "Can't calculate distance between A [%f ; %f] and B [%f ; %f].";
    static final Double EARTH_RADIUS_KM = 6372.795; //in km
    static final Integer METERS_IN_KM = 1000;
    static MXNavApp instance;
    ...
    private static Double getDistance(Double lat1, Double lon1, Double lat2, Double lon2) {
        try {
            ...
            MCLoggerFactory.getLogger().debug(String.format(FORMATTED_STRING, lat1, lon1, lat2, lon2, length));
            return length;
        } catch (Exception e) {
            MCLoggerFactory.getLogger().error(String.format(FORMATTED_EXCEPTION, lat1, lon1, lat2, lon2), e);
            return -1.0;
        }
    }

    protected void onTomTomError(final NavAppError navAppError) {
    ...
    public void planTrip(double lat, double lon, Trip.PlanListener listener) {
        ...
        Toast.makeText(MxApplication.getContext(), R.string.mx_tomtom_route, Toast.LENGTH_LONG).show();
    }
