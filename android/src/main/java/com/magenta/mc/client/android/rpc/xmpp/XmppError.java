...
public class XmppError {
    ...
    public final static String NOT_AUTHORIZED_TEXT = "not-authorized";
    ...
    public XmppError(int condition, String text) {
        ...
        switch (condition) {
            ...
            case NOT_AUTHORIZED:
                errorType = TYPE_AUTH;
                textCondition = NOT_AUTHORIZED_TEXT;
                break;
    ...
    public static XmppError findInStanza(XMLDataBlock stanza) {
        return decodeStanzaError(stanza.getChildBlock("error"));
    }

    public static XmppError decodeStanzaError(XMLDataBlock error) {
    ...
    public static XmppError decodeStreamError(XMLDataBlock error) {
    ...
    public static XmppError decodeSaslError(XMLDataBlock error) {
    ...
    private static XmppError decodeError(XMLDataBlock error, String ns) {
        int errCond = NONE;
        ...
        if (errChilds != null) for (Enumeration e = errChilds.elements(); e.hasMoreElements(); ) {
            ...
            if (tag.equals(NOT_AUTHORIZED_TEXT)) errCond = NOT_AUTHORIZED;
            ...
            try {
                ...
                switch (code) {
                    ...
                    case 400:
                        ...
                        break;
                    case 401:
                        errCond = NOT_AUTHORIZED;
                        break;
                    case 402:
                        errCond = PAYMENT_REQUIRED;
                        break;
                    case 403:
                        errCond = FORBIDDEN;
                        break;
                    case 404:
                        errCond = ITEM_NOT_FOUND;
                        break;
                    case 405:
                        errCond = NOT_ALLOWED;
                        break;
                    case 406:
                        errCond = NOT_ACCEPTABLE;
                        break;
                    case 407:
                        errCond = REGISTRATION_REQUIRED;
                        break;
                    case 408:
                        errCond = REMOTE_SERVER_TIMEOUT;
                        break;
                    case 409:
                        errCond = CONFLICT;
                        break;
                    case 500:
                        errCond = INTERNAL_SERVER_ERROR;
                        break;
                    case 501:
                        errCond = FEATURE_NOT_IMPLEMENTED;
                        break;
                    case 502:
                    case 503:
                    case 510:
                        errCond = SERVICE_UNAVAILABLE;
                        break;
                    case 504:
                        errCond = REMOTE_SERVER_TIMEOUT;
                        break;
                    default:
                        errCond = UNDEFINED_CONDITION;
                }
            } catch (Exception e) {
                errCond = UNDEFINED_CONDITION;
            }
    ...
    public XMLDataBlock construct() {
        if (errCondition == NONE) return null;
        XMLDataBlock error = new XMLDataBlock("error", null, null);
        String type = null;
        switch (errorType) {
            case TYPE_MODIFY:
                type = "modify";
        ...
        if (text != null)
            error.addChildNs("text", "urn:ietf:params:xml:ns:xmpp-stanzas").setText(text);
    ...
    public String getName() {
        return textCondition; /*TODO: remove stub*/
    }
